        ...
        {foreach from = $products item = produto}
            ...
            <div class="col-sm-3 colunaDestaque">
                ...
                <div id="prod{$count}" onmouseover="highlightItem({$count});" onmouseout="highlightItem({$count});" class="well well-sm col-xs-12 imgDiv pull-left">
                    <a onmouseover="highlightItem({$count});" onmouseout="highlightItem({$count});" href="{$templateRoot}pages/restaurant?res={$restaurants->get($count)->getId()}#{$produto->getNome()|replace:' ':''}">
                ...
                <div class="info">
                    <a class="restaurantName pull-left" href="{$templateRoot}pages/restaurant?res={$restaurants->get($count)->getId()}">{$restaurants->get($count)->getNome()}</a>
                    <a id="btProd{$count}" onmouseover="highlightItem({$count});" onmouseout="highlightItem({$count});" href="{$templateRoot}pages/restaurant?res={$restaurants->get($count)->getId()}#{$produto->getNome()|replace:' ':''}" class="btn btn-danger btn-block pull-left pecaAgora">Peça agora</a>
                </div>
            </div> 
            {$count = $count+1}
        {/foreach}
